  ...
  <div class="bookList">
        ...
        <template #default="{ row }">
          <div class="operation">
            <el-button
              v-if="row.stockNum !== 0"
              type="primary"
              @click="openBorrowBookDialog(row.id)"
              >借阅</el-button
            >
    ...
    <el-dialog v-model="borrowBookVisible" title="借阅图书" :draggable="true" destroy-on-close>
      ...
      <div class="borrowDialog">
        <div>是否借阅该图书?</div>
        <div>
          选择借阅的天数：<el-input-number
            v-model="borrowBookForm.borrowTime"
            :min="1"
            :max="30"
            size="small"
          ></el-input-number>
...
export default {
  ...
  setup() {
    ...
    function borrowBook() {
      borrowBookApi(markRaw(borrowBookForm)).then((res) => {
        // 借阅成功
    ...
    function reserveBook() {
      reserveBookApi(reserveBookId.value).then((res) => {
        ElMessage({ type: 'success', message: res.message, showClose: true, duration: 5000 });
        // 刷新列表
        getPageBookInfo();
        // 关闭对话框
