      ...
      <el-tabs type="border-card" v-model="currentTabName">
        <el-tab-pane label="登录" lazy name="login">
          ...
          >
            ...
            <el-button class="custom-button" style="width: 100%" type="primary" @click="login"
              >登录</el-button
            >
          </el-form>
        </el-tab-pane>
        <el-tab-pane label="注册" lazy name="register">
          ...
          <el-button class="custom-button" style="width: 100%" type="primary" @click="register"
            >注册</el-button
          >
        </el-tab-pane>
      </el-tabs>
  ...
  setup() {
    ...
    const login = () => {
      loginFormRef.value.validate((isValid) => {
        if (isValid) {
          loginApi(markRaw(loginForm)).then((res) => {
    ...
    const registerFormRules = {
      ...
      email: [
        { required: true, trigger: 'blur', message: '邮箱格式不能为空' },
    ...
    function register() {
      registerFormRef.value.validate((isValid) => {
        if (isValid) {
          registerApi(markRaw(registerForm)).then((res) => {
...
.login-wrapper {
  width: 100%;
  height: 100%;
  background-image: linear-gradient(to right, #c2fbcb, #a6c1ee);
  display: flex;
  align-items: center;
...
.custom-button {
  width: 100%;
  background-image: linear-gradient(to right, #a6c1ee, #c2fbcb);
  border: none;
}
