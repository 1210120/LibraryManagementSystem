  ...
  <el-breadcrumb separator="/" class="breadcrumb-wrapper">
    <transition-group name="breadcrumb" mode="out-in">
      <el-breadcrumb-item v-for="(item, index) in crumbList" :key="item.path">
        <router-link v-if="index === 0" :to="{ path: item.path }">
          {{ item.meta.title }}
        </router-link>
  ...
  setup() {
    ...
    function getBreadcrumb() {
      ...
      let matched = route.matched.filter(
        (item) => item.path && item.meta && item.meta.title && item.meta.breadcrumb !== false
      );
      let crumbIndex = 0;
      let matchedIndex = 0;
      ...
      // crumbList.splice(crumbIndex, 1, ...matched.slice(matchedIndex));
    }
    // 判断一个路由对象是否代表的为首页
