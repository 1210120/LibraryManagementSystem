...
export default function (chartDomRef, getOptions) {
  // 判断参数是否满足要求,getOptions类型是异步函数
  // if (isRef(chartDomRef) && Object.prototype.toString.call(getOptions) === '[object Function]') {
  if (isRef(chartDomRef) && Object.prototype.toString.call(getOptions) === '[object AsyncFunction]') {
    // 图表对象
    let chart = null
    // 监听图表容器大小改变的检测器
    ...
    console.log("chartDomRef是Ref对象:" + isRef(chartDomRef));
    console.log("getOptions类型:" + Object.prototype.toString.call(getOptions));
    console.log("drawChartHook没有调用");
  }
}
